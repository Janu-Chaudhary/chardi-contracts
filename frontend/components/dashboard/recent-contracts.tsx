  ...
  return (
    <section aria-labelledby="recent-heading">
      <div className="mb-4 flex items-end justify-between gap-4">
        ...
        <Link
          href="/contracts"
          className="interactive group hidden items-center gap-1 text-sm font-medium text-coral-600 hover:text-coral-600 sm:inline-flex"
        >
          View all
      ...
      <Card className="hidden md:block">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <Link
            href="/contracts"
            className="interactive text-sm font-medium text-coral-600 hover:text-coral-600/90"
          >
            View all →
          </Link>
        </CardHeader>
        <CardContent className="divide-y divide-border p-0">
          {contracts.map((c) => (
            <Link
              key={c.id}
              href={`/contracts/${c.id}`}
              className="interactive-row group flex items-center justify-between gap-4 px-6 py-4"
            >
              <div className="min-w-0">
                <p className="truncate font-medium text-warm-black transition-colors group-hover:text-coral-600">
                  {c.title}
                </p>
      ...
      <Link
        href="/contracts"
        className="interactive mt-4 flex h-11 w-full items-center justify-center rounded-lg border border-border text-sm font-medium hover:border-warm-300 hover:bg-warm-50 sm:hidden"
      >
        View all contracts
      </Link>
    </section>
