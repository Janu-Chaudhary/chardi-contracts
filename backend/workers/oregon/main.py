"""
Oregon OregonBuys ingestion entry point.
...
from backend.workers.oregon.fetcher import fetch_oregon_records
from backend.workers.oregon.mapper import SOURCE_PORTAL, map_oregon_row

PORTAL_LABEL = "Oregon OregonBuys — Purchases and Contracts"


async def run_oregon_ingestion() -> dict[str, object]:
    ...
    try:
        print(f"Run {run_id}: Starting Oregon ingestion...")
        async with aiohttp.ClientSession(headers={"User-Agent": settings.USER_AGENT}) as session:
            raw_records = await fetch_oregon_records(session)

        if not raw_records:
            status = "SUCCESS"
        else:
            mapped = [map_oregon_row(r) for r in raw_records]
        ...
        print(f"[ERROR] Oregon run failed: {error_msg}")
...
async def _async_main() -> None:
    ...
    try:
        result = await run_oregon_ingestion()
